...
const SearchBarJa = forwardRef<SearchBarRef, SearchBarProps>(({ 
  onSearch,
  availableSkills,
  availableProjects
}, ref) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <div className="w-full sm:w-1/2 md:w-1/6">
          ...
          <div className="relative">
            ...
            >
              <option value="" className="text-gray-400 text-sm">すべての言語</option>
        ...
        <div className="w-full sm:w-1/2 md:w-1/6">
          ...
          <div className="relative">
            <select
              ...
              className="w-full px-3 py-2.5 border border-gray-200 rounded-lg bg-white text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent dark:bg-gray-800 dark:border-gray-700 dark:text-white appearance-none"
            >
              <option value="" className="text-gray-400 text-sm">すべてのプロジェクト</option>
      ...
      <div className="text-xs text-gray-400 dark:text-gray-500">
        {selectedSkill || selectedProject ? (
          <span>
            フィルター条件: {selectedSkill && <span className="font-medium text-gray-600 dark:text-gray-300">{selectedSkill}</span>}
            {selectedSkill && selectedProject && <span> と </span>}
            {selectedProject && <span className="font-medium text-gray-600 dark:text-gray-300">{selectedProject}</span>}
          </span>
        ) : (
          <span>フィルターを選択してエンジニアリストを絞り込んでください</span>
        )}
      </div>
    </div>
  );
...
SearchBarJa.displayName = "SearchBarJa";

export default SearchBarJa;
